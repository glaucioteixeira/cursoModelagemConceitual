...
public interface ProdutoRepository extends JpaRepository<Produto, Integer> {
	...
	//@Query("SELECT DISTINCT obj FROM Produto obj INNER JOIN obj.categorias cat WHERE UPPER(obj.nome) LIKE %:nome% AND cat IN :categorias")
	//Page<Produto> search(@Param("nome") String nome, @Param("categorias") List<Categoria> categorias, Pageable pageRequest);
	
	// jpa.query-methods :: https://docs.spring.io/spring-data/jpa/docs/current/reference/html/
